...
OUTPUT_FILE = 'Germany_Imputation_Results.xlsx'
...
def run_regression(df, formula, title):
    ...
    out = f"\n\n{'-'*20}\n{title}\n{'-'*20}\n"
    try:
        # Check for infs before regression
        if np.isinf(df.select_dtypes(include=np.number)).values.any():
            logger.warning("Found Infinite values in data! Dropping them...")
...
def prepare_data_and_analyze(df, tax_col, method='CC'):
    ...
    # *** FIX: Treat 0 as NaN for Imputation/Log purposes ***
    # Many rows have 0 employees or 0 assets, which causes log(0) = -inf later.
    # We replace 0 with NaN so the Imputer fills them, OR we drop them if using CC.
    ...
    if method == 'CC': 
        ...
        imputer = IterativeImputer(min_value=0.1, max_iter=20, random_state=42) # min_value=0.1 avoids zeros
    ...
    # 3. Logs (With safety check)
    # Ensure no values <= 0 before log
    ...
    logger.info(f"[{method}] Final Sample Size: {len(data)}")
